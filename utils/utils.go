...
func MakeCoTIteration(url string, api_key string, model string, query string) (*string, error) {
	...
	finalRequestBody := types.ChatCompletionsRequest{
		Model:       model,
		MaxTokens:   300,
		Temperature: 0.2,
		ResponseFormat: types.ResponseFormat{
	...
	finalRequestBody.Messages = append(finalRequestBody.Messages, types.Message{
		Role:    "user",
	...
	// Marshal the payload to JSON
	jsonData, err := json.Marshal(finalRequestBody)
	if err != nil {
		log.Error("Error marshalling JSON:", err)
		return nil, err
	}
